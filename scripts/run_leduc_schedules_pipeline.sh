#!/bin/bash
# Parallel Pipeline for Leduc Poker Schedule Experiments
#
# This script:
# 1. Starts all 4 schedule training jobs in parallel
...
# 5. Provides real-time status updates with timing
...
echo "=========================================="
echo "Leduc Poker Schedule Experiments Pipeline"
echo "=========================================="
echo ""
echo "Testing 4 different alpha warmup schedules:"
echo "  1. Linear"
echo "  2. Exponential (λ=0.3)"
echo "  3. Exponential (λ=0.5)"
echo "  4. Cosine"
echo ""
...
# Configuration
EXPERIMENTS=("schedule_linear" "schedule_exp03" "schedule_exp05" "schedule_cosine")
NAMES=("Linear Schedule" "Exponential (λ=0.3)" "Exponential (λ=0.5)" "Cosine Schedule")

# Time estimates (in seconds)
EXPECTED_TRAIN_TIME=126    # ~2.1 minutes (35k steps)
...
echo "  1. Training (4 schedules in parallel) - ETA: $(format_duration $EXPECTED_TRAIN_TIME) each"
echo "  2. Exploitability (starts as each training completes) - ETA: $(format_duration $EXPECTED_EXPLOIT_TIME) each"
echo ""
echo "Expected total time: ~17 minutes (with overlapping exploits)"
echo ""
...
run_training() {
    ...
    echo -e "${BLUE}[TRAIN]${NC} Progress will be shown below..."
    echo ""

    # Use tee to show output AND save to log
    ./scripts/train_leduc_${exp}.sh 2>&1 | tee logs/train_leduc_${exp}.log &
    local pid=$!
    ...
    echo ""
    echo -e "${BLUE}[TRAIN]${NC} $name → PID $pid (output above)"
}
...
run_exploit() {
    ...
    echo -e "${YELLOW}[EXPLOIT]${NC} Progress will be shown below..."
    echo ""

    # Use tee to show output AND save to log
    ./scripts/compute_exploit_leduc_${exp}.sh 2>&1 | tee logs/exploit_leduc_${exp}.log &
    local pid=$!
    ...
    echo ""
    echo -e "${YELLOW}[EXPLOIT]${NC} $name → PID $pid (output above)"
}
...
echo "  Logs: logs/leduc_poker/nash_pg/schedule_*.json"
echo "  Checkpoints: checkpoints/leduc_poker/nash_pg/schedule_*"
echo "  TensorBoard: runs/leduc_poker/nash_pg/schedule_*"
echo ""

if [ $failure_count -eq 0 ]; then
    echo -e "${GREEN}All schedule experiments completed successfully! 🎉${NC}"
    exit 0
else
