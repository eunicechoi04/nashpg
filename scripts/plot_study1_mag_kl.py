...
Plots mag_kl = KL(π || ρ) over time across multiple log files.
"""
...
def plot_mag_kl(log_files, output_file=None, labels=None):
    """
    Plot mag_kl vs training steps for multiple log files.
    ...
    for i, log_file in enumerate(log_files):
        ...
        plt.plot(steps, mag_kl, label=label, alpha=0.7)
    
    plt.xlabel('Training Steps')
    plt.ylabel('mag_kl = KL(π || ρ)')
    plt.title('Study 1: Reference Policy Stability - mag_kl vs Training Steps')
    plt.legend()
...
def main():
    parser = argparse.ArgumentParser(description='Plot mag_kl vs training steps')
    ...
    plot_mag_kl(args.log_files, args.output, args.labels)
